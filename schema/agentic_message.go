...
type ContentBlock struct {
	...
	// FunctionToolCall 包含用户定义工具的调用详情。
	...
	// FunctionToolResult 包含用户定义工具调用返回的结果。
	...
	//
	// ToolSearchFunctionToolResult 包含客户端自定义工具搜索工具调用的结果。
	// 它携带新发现工具的完整定义，以便模型识别已添加且现在可调用的工具。
	...
	// ServerToolCall 包含在模型服务器上执行的 provider 内置工具的调用详情。
	...
	// ServerToolResult 包含在模型服务器上执行的 provider 内置工具返回的结果。
	...
	// MCPToolCall 包含由模型服务器管理的 MCP 工具的调用详情。
	...
	// MCPToolResult 包含由模型服务器管理的 MCP 工具返回的结果。
	...
	// MCPListToolsResult 包含模型服务器报告的可用 MCP 工具列表。
	...
	// MCPToolApprovalRequest 包含 MCP 工具调用在需要时的用户批准请求。
	...
	// MCPToolApprovalResponse 包含用户对 MCP 工具调用的批准决定。
...
type UserInputImage struct {
	...
	// Base64Data 是 Base64 编码字符串格式的二进制数据。
	Base64Data string `json:"base64_data,omitempty"`

	// MIMEType is the mime type, e.g. "image/png".
	// MIMEType 是 mime type，例如 "image/png"。
...
type UserInputAudio struct {
	...
	// MIMEType 是 mime type，例如 "audio/wav"。
	MIMEType string `json:"mime_type,omitempty"`
}
...
type UserInputVideo struct {
	...
	// MIMEType 是 mime type，例如 "video/mp4"。
	MIMEType string `json:"mime_type,omitempty"`
}
...
type UserInputFile struct {
	...
	// MIMEType 是 mime type，例如 "application/pdf"。
	MIMEType string `json:"mime_type,omitempty"`
}
...
type AssistantGenImage struct {
	...
	// MIMEType is the mime type, e.g. "image/png".
	// MIMEType 是 mime 类型，例如 "image/png"。
	MIMEType string `json:"mime_type,omitempty"`
}
...
type AssistantGenAudio struct {
	...
	// MIMEType is the mime type, e.g. "audio/wav".
	// MIMEType 是 mime 类型，例如 "audio/wav"。
	MIMEType string `json:"mime_type,omitempty"`
}
...
//
// FunctionToolResultContentBlockType 标识 FunctionToolResultContentBlock 中填充了哪个媒体字段。
...
//
// FunctionToolResultContentBlock 表示多模态函数工具结果中的单个内容块。
// Type 标识填充了哪个媒体字段；应只设置一个与 Type 匹配的媒体字段。
...
type FunctionToolResult struct {
	...
	//
	// Content 以有序内容块列表保存工具执行输出。
	// 每个块都有自己的类型（text、image、audio、video、file），使纯文本和多模态结果可共用统一表示。
...
//
// ToolSearchFunctionToolResult 表示客户端自定义工具搜索函数工具调用的结果。
// 与常规 FunctionToolResult 不同，它携带包含新发现工具完整定义的 ToolSearchResult，
// 让模型能识别已添加且现在可调用的工具。
...
type ServerToolCall struct {
	...
	//
	// Name 指定被调用的服务端工具。
	// 由模型服务器提供（例如 OpenAI 的 `web_search`，Gemini 的 `googleSearch`）。
	Name string `json:"name"`
...
type MCPToolCall struct {
	...
	// ServerLabel 是 MCP 服务器标签，用于在工具调用中标识它。
	ServerLabel string `json:"server_label,omitempty"`

	// ApprovalRequestID is the approval request ID.
	// ApprovalRequestID 是审批请求 ID。
	...
	// Arguments 是工具调用参数的 JSON 字符串。
	Arguments string `json:"arguments,omitempty"`
}

type MCPToolResult struct {
	// ServerLabel is the MCP server label used to identify it in tool calls
	// ServerLabel 是用于在工具调用中标识 MCP 服务器的标签。
	ServerLabel string `json:"server_label,omitempty"`
...
type MCPListToolsItem struct {
	...
	// InputSchema 是描述工具输入参数的 JSON schema。
...
// SystemAgenticMessage 表示 AgenticRoleType 为 "system" 的消息。
...
// UserAgenticMessage 表示 AgenticRoleType 为 "user" 的消息。
...
// NewContentBlock 使用给定 content 创建新的 ContentBlock。
...
// NewContentBlockChunk 使用给定 content 和流式元数据创建新的 ContentBlock。
...
//
// AgenticMessagesTemplate 是 agentic messages template 的接口。
// 用于将模板渲染为 agentic messages 列表。
// 例如：
// chatTemplate := prompt.FromAgenticMessages(
// &schema.AgenticMessage{
// Role: schema.AgenticRoleTypeSystem,
...
// },
// },
// schema.AgenticMessagesPlaceholder("history", false), // <= 这里会使用 params 中 "history" 的值
// )
// msgs, err := chatTemplate.Format(ctx, params)
...
//	msgs, err := chatTemplate.Format(ctx, params)
//
// AgenticMessagesPlaceholder 可将占位符渲染为 params 中的 agentic messages 列表。
// 例如：
// placeholder := AgenticMessagesPlaceholder("history", false)
// params := map[string]any{
// "history": []*schema.AgenticMessage{
...
//
// Format 返回按给定 formatType 渲染后的 agentic messages。
// 它只格式化用户输入字段（UserInputText, UserInputImage, UserInputAudio, UserInputVideo, UserInputFile）。
// 例如：
// msg := &schema.AgenticMessage{
// Role: schema.AgenticRoleTypeUser,
// ContentBlocks: []*schema.ContentBlock{
...
// }
// msgs, err := msg.Format(ctx, map[string]any{"name": "eino"}, schema.FString)
// msgs[0].ContentBlocks[0].UserInputText.Text 将是 "hello eino"
...
// ConcatAgenticMessagesArray 将多个 AgenticMessage 流拼接为单个 AgenticMessage slice。
...
// ConcatAgenticMessages 将一组 AgenticMessage chunk 拼接为单个 AgenticMessage。
...
//
// concatContentBlockHelper 是一个通用辅助函数，用于减少拼接特定类型 content blocks 时的代码重复。
...
// String 返回 AgenticMessage 的字符串表示。
...
// String 返回 UserInputAudio 的字符串表示。
...
// String 返回 FunctionToolResult 的字符串表示。
...
// String 返回 ServerToolCall 的字符串表示。
...
// String 返回 ServerToolResult 的字符串表示。
...
// String 返回 MCPToolResult 的字符串表示。
...
// String 返回 MCPListToolsResult 的字符串表示。
...
// String 返回 MCPToolApprovalRequest 的字符串表示。
...
// String 返回 MCPToolApprovalResponse 的字符串表示。
...
// String 返回 AgenticResponseMeta 的字符串表示。
...
// truncateString 将字符串截断为最多 maxLen 个字符，若被截断则添加 "..."
...
// formatMediaString 格式化媒体内容的 URL、Base64Data、MIMEType 和 Detail
