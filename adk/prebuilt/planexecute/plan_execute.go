...
// Package planexecute 实现了 plan–execute–replan 风格的智能体。
package planexecute
...
// "type": "string"
// },
// "description": "要执行的有序操作列表。每个步骤都应清晰、可执行，并按逻辑顺序排列。"
// }
// },
// "required": ["steps"]
// }
...
var (
	...
	//
	// PlanToolInfo 定义可与 ToolCallingChatModel 一起使用的 Plan 工具 schema。
	...
	//
	// RespondToolInfo 定义可与 ToolCallingChatModel 一起使用的响应工具 schema。
	// 该 schema 指示模型生成给用户的直接响应。
	...
	//
	// PlannerPrompt 是规划器的提示模板。
	// 它为规划器生成 Plan 提供上下文和指导。
...
// 配置规划器生成结构化 Plan 输出有两种方式：
// 1. 使用 ChatModelWithFormattedOutput：预配置为输出 Plan 格式的模型
// 2. 使用 ToolCallingChatModel + ToolInfo：通过工具调用生成 Plan 结构的模型
type PlannerConfig struct {
	...
	//
	// ChatModelWithFormattedOutput 是预配置为输出 Plan 格式的模型。
	// 可通过配置模型直接输出结构化数据来创建。
	// 示例见：https://github.com/cloudwego/eino-ext/blob/main/components/model/openai/examples/structured/structured.go
	...
	// 提供 ToolInfo 时，它会使用工具调用生成 Plan 结构。
	...
	//
	// GenInputFn 是为 planner 生成输入消息的函数。
	// 可选。若未提供，将使用 defaultGenPlannerInputFn。
...
// to generate structured Plan output.
//
// NewPlanner 基于提供的配置创建新的 planner 智能体。
// planner 智能体使用 ChatModelWithFormattedOutput 或 ToolCallingChatModel+ToolInfo
// 来生成结构化的 Plan 输出。
// 如果提供了 ChatModelWithFormattedOutput，将直接使用它。
// 如果提供了 ToolCallingChatModel，将使用 ToolInfo（或默认的 PlanToolInfo）对其进行配置
// 以生成结构化的 Plan 输出。
...
// GenModelInputFn 是为 executor 和 planner 生成输入消息的函数。
...
// ExecutorConfig 提供创建 executor 智能体的配置选项。
type ExecutorConfig struct {
	...
	//
	// Model 是 executor 使用的聊天模型。
	// 如果 executor 使用任何工具，该模型必须支持 model.WithTools 调用选项，
	// 因为 executor 正是通过它使用工具信息配置模型。
	Model model.BaseChatModel

	// ToolsConfig specifies the tools available to the executor.
	// ToolsConfig 指定 executor 可用的工具。
	...
	//
	// GenInputFn 为 Executor 生成输入消息。
	// 可选。若未提供，将使用 defaultGenExecutorInputFn。
...
type ReplannerConfig struct {
	...
	// 它将配置 PlanTool 和 RespondTool，以生成更新后的计划或响应。
	ChatModel model.ToolCallingChatModel
	...
	//
	// PlanTool 定义可与 ToolCallingChatModel 一起使用的 Plan 工具 schema。
	// 可选。若未提供，将使用默认的 PlanToolInfo。
	PlanTool *schema.ToolInfo
	...
	//
	// RespondTool 定义可与 ToolCallingChatModel 一起使用的响应工具 schema。
	// 可选。若未提供，将使用默认的 RespondToolInfo。
	RespondTool *schema.ToolInfo

	// GenInputFn generates the input messages for the Replanner.
	// Optional. If not provided, buildGenReplannerInputFn will be used.
	//
	// GenInputFn 为 Replanner 生成输入消息。
	// 可选。若未提供，将使用 buildGenReplannerInputFn。
	GenInputFn GenModelInputFn
	...
	// 返回的 Plan 将用于反序列化 PlanTool 中模型生成的 JSON 输出。
	// 可选。若未提供，将使用 defaultNewPlan。
	NewPlan NewPlan
}
...
//
// NewReplanner 创建一个连接了 plan 和 respond 工具的 plan-execute-replan 智能体。
// 它使用这些工具配置提供的 ToolCallingChatModel，并返回一个 Agent。
...
type Config struct {
	...
	//
	// Executor 指定执行 planner 或 replanner 生成的计划的智能体。
	// 可以使用提供的 NewExecutor 创建 executor 智能体。
	Executor adk.Agent
	...
	// 可以使用提供的 NewReplanner 创建 replanner 智能体。
	Replanner adk.Agent
	...
	//
	// MaxIterations 定义 'execute-replan' 的最大循环次数。
	// 可选。未提供时默认使用 10。
	MaxIterations int
}
...
//
// New 使用给定配置创建新的 plan-execute-replan 智能体。
// plan-execute-replan 模式分为三个阶段：
// 1. Planning：生成结构化计划，包含清晰、可执行的步骤
// 2. Execution：执行计划的第一步
// 3. Replanning：评估进展，并完成任务或修订计划
// 这种方法通过迭代优化支持复杂问题求解。
